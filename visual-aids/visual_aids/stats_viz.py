...
def anscombes_quartet(r_squared=False):
    ...
    # get data
    anscombe = pl.from_pandas(sns.load_dataset('anscombe')).group_by('dataset')
    ...
    for ax, (group_name, group_data), title in zip(axes, anscombe, titles):
        # get x, y
        x, y = group_data.get_column('x'), group_data.get_column('y')
        ...
        if r_squared:
            ax.annotate(
                f"""ρ = {np.corrcoef(x,y)[0][1]:.2f}\ny = {m:.2f}x + {b:.2f}\n\n{
                    ...
                    r'$μ_y$'
                } = {y.mean():.2f} | {r'$σ_y$'} = {y.std():.2f}""", xy=(13, 2.5)
            )
        else:
            ax.annotate(
                f"""ρ = {np.corrcoef(x,y)[0][1]:.2f}\ny = {m:.2f}x + {b:.2f}\n\n{
...
def datasaurus_dozen():
    ...
    """
    df = pl.read_csv(pkg_resources.resource_stream(__name__, 'data/DatasaurusDozen.tsv'), separator='\t')
    ...
    # plot the Datasaurus
    data = df.filter(pl.col('dataset') == 'dino')
    axes[1].scatter(
        x=data.get_column('x'),
        y=data.get_column('y'),
    )
    axes[1].set_title('dino')
    
    # calculate the summary statistics
    x, y = data.get_column('x'), data.get_column('y')
    axes[2].text(
        s=f"""ρ  = {np.corrcoef(x,y)[0][1]:.2f}\n{
            ...
            r'$μ_y$'
        } = {y.mean():.2f}\n{r'$σ_y$'} = {y.std():.2f}""", x=0.5, y=0.5, fontsize=20, fontfamily='DejaVu Sans Mono'
    )
    ...
    for (title, data), ax in zip(df.filter(pl.col('dataset') != 'dino').group_by('dataset'), axes[4:]):
        ax.scatter(
            x=data.get_column('x'),
            y=data.get_column('y'),
        )
...
def example_regression():
    ...
    plt.title("Using regression to predict ice cream sales")
    ax.scatter(
        x=ice_cream_sales.get_column('temps'),
        y=ice_cream_sales.get_column('sales'),
    )
    plt.xlim((15, 45))
    plt.ylim((0, 40))

    # plot regression line
    m, b = np.polyfit(ice_cream_sales.get_column('temps'), ice_cream_sales.get_column('sales'), 1)
    reg_x = ice_cream_sales.get_column('temps')
    reg_y = [m*num + b for num in reg_x]
...
def example_scatter_plot():
    ...
    f, ax = plt.subplots(figsize=(12, 5))
    ax.scatter(
        x=ice_cream_sales.get_column('temps'), y=ice_cream_sales.get_column('sales'),
    )

    plt.xlim((15, 45))
    plt.ylim((0, 40))
    plt.title('ice cream sales at a given temperature')
